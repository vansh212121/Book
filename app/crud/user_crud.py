...
# T = TypeVar("T")
# # DB_ERROR = InternalServerError("An unexpected database error occurred.")
...
#     # @handle_exceptions(default_exception=InternalServerError, message="An unexpected database error occurred.")
...
class UserRepository(BaseRepository[User]):
    ...
    async def create_from_model(self, db: AsyncSession, *, db_obj: User) -> User:
        """Create a new user from User model (for tests/fixtures)."""
        db.add(db_obj)
        await db.commit()
        await db.refresh(db_obj)
